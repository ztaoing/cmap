...
func TestRemoveCb(t *testing.T) {
	...
	result := m.RemoveCallBack("monkey", cb)
	if !result {
	...
	result = m.RemoveCallBack("elephant", cb)
	if result {
	...
	result = m.RemoveCallBack("horse", cb)
	if result {
		t.Errorf("Result was true")
	}
...
func TestIterCb(t *testing.T) {
	...
	counter := 0
	// Iterate over elements.
	m.IterCallback(func(key string, v interface{}) {
		_, ok := v.(Animal)
		if !ok {
